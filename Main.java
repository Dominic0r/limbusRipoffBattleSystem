...
public class Main
{
    ...
    public static class StatusDTO {
        public String id, name, description, trigger, action;
    ...
    public static class MoveDTO {
        ...
        public int baseAtk;
        public List<CoinDTO> coins = new ArrayList<>();
    }
    
    public static class UnitDTO {
        public String id, name, description;
        public int hp, morale, speed, stagger;
        public List<MoveDTO> moves = new ArrayList<>();
    }
    
    public static class PackageDTO {
        public List<StatusDTO> statusEffects = new ArrayList<>();
    ...
    public static class ContentLoader {
        ...
        public static void loadDirectory(String folderPath) {
            Path dirPath = Paths.get(folderPath);
            
            // Ensure directory exists
            if (!Files.exists(dirPath) || !Files.isDirectory(dirPath)) {
                System.out.println("Warning: Mods directory not found at " + folderPath);
                return;
            }
    
            try (DirectoryStream<Path> stream = Files.newDirectoryStream(dirPath, "*.json")) {
                for (Path entry : stream) {
                    System.out.println("Loading content package: " + entry.getFileName());
                    loadSingleJsonFile(entry.toString()); // Parse single JSON file
                }
            } catch (Exception e) {
        ...
        public static void loadSingleJsonFile(String filePath) {
            try (Reader reader = new FileReader(filePath)) {
                // 1. Parse JSON directly into DTO structure
                PackageDTO pkg = gson.fromJson(reader, PackageDTO.class);
    
                if (pkg == null) return;
    
                // 2. Load Status Effects into Registry
                if (pkg.statusEffects != null) {
                    for (StatusDTO sDto : pkg.statusEffects) {
                        statusEffect effect = createStatus(
                            sDto.name, 
                            sDto.decays, 
                            sDto.limit, 
                            sDto.description, 
                        ...
                        Registry.registerStatus(sDto.id, effect);
                    }
                }
    
                // 3. Load Unit Templates into Registry
                if (pkg.units != null) {
                    for (UnitDTO uDto : pkg.units) {
                        List<Move> moveSet = new ArrayList<>();
                        
                        for (MoveDTO mDto : uDto.moves) {
                            Move move = new Move(mDto.name, mDto.baseAtk, mDto.description);
                            if (mDto.coins != null) {
                                for (CoinDTO cDto : mDto.coins) {
                                    move.addCoin(new Coin(cDto.power, cDto.desc));
                                }
                            }
                            moveSet.add(move);
                        }
                        ...
                        Registry.PRESET_UNITS.put(uDto.id.toLowerCase(), unitTemplate);
                    }
                }
        ...
        // Instantiates a clean Unit copy from registry templates to avoid shared state bugs
        public static Unit createUnit(String unitId) {
            Unit template = Registry.PRESET_UNITS.get(unitId.toLowerCase());
            if (template == null) {
                throw new IllegalArgumentException("Unit ID not found in registry: " + unitId);
            }
    
            List<Move> clonedMoves = new ArrayList<>();
            for (Move move : template.getMoveSet()) {
                Move newMove = new Move(move.getName(), move.getBaseAtk(), move.getDesc());
                for (Coin coin : move.getCoinSet()) {
                    newMove.addCoin(new Coin(coin.getAtkPoints(), coin.getDesc()));
                }
                clonedMoves.add(newMove);
            }
        ...
        // Helper builder for preset status effects
        public static statusEffect createStatus(String name, boolean decays, int limit, String desc, String trigger, String action) {
            statusEffect effect = new statusEffect(name, decays, limit, desc);
            
            BiConsumer<Battlefield, Unit> hook = (field, un) -> {
                for (appliedEffect app : new ArrayList<>(un.getEffectList())) {
                    if (app.stat() == effect) {
                        if ("DEAL_POTENCY_DAMAGE".equalsIgnoreCase(action)) {
                            int dam = app.getPotency();
                            un.takeHPDamage(dam);
                            app.decrementStack();
                            System.out.println("  > " + un.getName() + " took " + dam + " " + name + " damage!");
                        } else if ("LOSE_MORALE".equalsIgnoreCase(action)) {
                            un.modifyMorale(-app.getPotency());
                            app.decrementStack();
                        }
                    }
                }
            ...
            switch (trigger.toUpperCase()) {
                case "ON_CLASH": effect.setOnClash(hook); break;
                case "ON_TURN_START": effect.setOnTurnStart(hook); break;
                case "ON_TURN_END": effect.setOnTurnEnd(hook); break;
                case "ON_HIT_RECEIVED": effect.setOnHitReceived(hook); break;
                case "ON_HIT_GIVE": effect.setOnHitGive(hook); break;
            }
            return effect;
        }
    }
    ...
    //==========================================================================================
    //Modding Zone. Anything outside these walls must be treated with precaution unless you know what you're doing
    public static void setupBattle(){
        //parses all content files
        ContentLoader.loadDirectory("./mods/");
        
        //Register player. 
        playerUnit = ContentLoader.createUnit("default_player");
        
        //Configure Ally Team
        allAllies.add(ContentLoader.createUnit("default_ally"));
        
        //Configure Enemy Team 
        allEnemies.add(ContentLoader.createUnit("default_enemy"));
    }
    //==========================================================================================
